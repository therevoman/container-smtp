...
echo -n "" > /etc/exim/exim4.conf.localmacros

if [ "$MAILNAME" ]; then
	echo "MAIN_HARDCODE_PRIMARY_HOSTNAME = $MAILNAME" > /etc/exim/exim4.conf.localmacros
	echo $MAILNAME > /etc/mailname
fi

if [ "$KEY_PATH" -a "$CERTIFICATE_PATH" ]; then
	if [ "$MAILNAME" ]; then
	  echo "MAIN_TLS_ENABLE = yes" >>  /etc/exim/exim4.conf.localmacros
	else
	  echo "MAIN_TLS_ENABLE = yes" >>  /etc/exim/exim4.conf.localmacros
	fi
...
if [ "$DISABLE_IPV6" ]; then
        echo 'disable_ipv6=true' >> /etc/exim/exim4.conf.localmacros
fi
...
if [ -f /etc/exim/_docker_additional_macros ]; then
	cat /etc/exim/_docker_additional_macros >> /etc/exim/exim4.conf.localmacros
fi
